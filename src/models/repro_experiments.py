...
from src.entities.train_pipeline_params import TrainingParams
...
def interpret_features(model, val_features):
	tempdf = pd.DataFrame(val_features, columns=val_features.columns).sample(frac=1)[:50]

	# create explainer to compute shap values
	explainer = shap.KernelExplainer(lambda x: model.predict(x), tempdf)

	# compute shap values for all xtext
...
def log_experiment_mlflow(
		...
		training_pipeline_params: TrainingParams
) -> Tuple[CatBoostClassifier, dict]:
	mlflow.set_tracking_uri("http://127.0.0.1:5000")
	mlflow.set_experiment("demo2")

	with mlflow.start_run(run_name=f"RUN_{run_name}") as run: # Здесь нужно иметь ввиду, что в коде Мунина был .format
